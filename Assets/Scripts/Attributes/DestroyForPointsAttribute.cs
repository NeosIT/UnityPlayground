	...
	private void OnTriggerEnter2D(Collider2D collisionData)
	{
		...
		if(collisionData.gameObject.CompareTag("Bullet"))
		{
			if(userInterface != null)
			{
				...
				{
					Debug.Log(_("Use a BulletAttribute on one of the objects involved in the collision if you want one of the players to receive points for destroying the target."));
				}
			}
			else
			{
				Debug.Log(_("There is no UI in the scene, hence points can't be displayed."));
			}
