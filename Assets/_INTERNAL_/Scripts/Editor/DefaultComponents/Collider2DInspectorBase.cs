...
public class Collider2DInspectorBase : Editor
{
	...
	protected string triggerMessage = _("A Collider marked as \"Trigger\" is a special type of collider that can't be touched by other things, but it still detects if another GameObject enters it.\nUseful for Area Condition scripts.");

	protected void ShowExtrasBlock(string[] properties)
	{
		showExtras = EditorGUILayout.Foldout(showExtras, new GUIContent(_("Extra Options")));
		if(showExtras)
		{
